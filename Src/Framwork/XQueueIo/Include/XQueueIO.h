...
int CXQueueIo<MSG>::Start()
{
	...
	if (m_uiMain == 1)
	{
		int nRet = m_Queue.Create((LPCTSTR)m_sXQueName.c_str(), m_uiXQueLen, m_uiBlockSize);
		if (nRet)
		{	
	...
	{
		int nRet = m_Queue.Open((LPCTSTR)m_sXQueName.c_str());
		if (!m_Queue.IsOpen())
		{	
...
int CXQueueIo<MSG>::Write(MSG& oMsg, unsigned int uiTimeout)
{
	try
	{
		...
		if (m_Queue.IsOpen())
		{
			...
			const char * pBuf = oMsg.Encode(iSize);
			if (iSize > 0)
			{
				nSize = iSize;
